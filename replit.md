...
- Payment orders and verified outcomes are stored in the Replit PostgreSQL `payment_transactions` table, keyed to the validated Firebase UID.
- Student personal and guardian details are stored in Replit PostgreSQL `student_profiles`, while Firebase remains responsible for authentication and Firestore continues to hold roles and learning progress. Existing Firestore profiles are copied to PostgreSQL on first profile load.
